...
function getVerifyEndpoint() {
    const meta = document.querySelector('meta[name="replay-access-verify-url"]');
...
function getDeviceStorageKey() {
    ...
    const tokenKey = payload?.jti || token || "replay";
    return `replay-device:${tokenKey}`;
}
...
function generateDeviceId() {
    ...
    return `dev-${Date.now().toString(36)}-${randomPart}`;
}
...
async function verifyReplayAccess() {
    const token = getAccessToken();

    if (!token) {
        lockPage("Token dibutuhkan", "Link replay ini butuh token akses. Minta link terbaru dari admin atau bot Telegram.");
        return false;
    }

    state.accessToken = token;
    state.accessPayload = decodeJwtPayload(token);
    const deviceId = getOrCreateDeviceId();
    const deviceLabel = getDeviceLabel();

    const verifyUrl = getVerifyEndpoint();
    if (!verifyUrl) {
        lockPage("Konfigurasi belum lengkap", "Endpoint verifikasi token belum diatur pada halaman replay.");
        return false;
    }
    ...
    try {
        ...
        lockPage("Verifikasi gagal", "Server verifikasi token tidak bisa dihubungi. Coba lagi beberapa saat atau minta link baru.");
        return false;
    }
    ...
    if (!response.ok || !result?.success || !result?.valid) {
        lockPage(
            "Akses ditolak",
            result?.message || "Token replay tidak valid atau sudah tidak bisa dipakai. Minta link baru dari bot Telegram."
        );
        return false;
    }
...
async function init() {
    ...
    updateProgressUI(0, 0);
    const isAuthorized = await verifyReplayAccess();
    if (!isAuthorized) {
        return;
    }
    await loadReplays();
